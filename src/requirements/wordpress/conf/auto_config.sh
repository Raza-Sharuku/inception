...
if [ ! -f /var/www/html/wordpress/wp-config.php ]; then
  ...
  wp core install --url="${SITE_DOMAIN_NAME}" --title="inception" --admin_user=${WP_ADMIN_NAME} --admin_password=${WP_ADMIN_PASSWD} --admin_email="admin@example.com" --allow-root

  # Create additional user
  wp user create "${WP_AUTHOR_NAME}" user@example.com --user_pass="${WP_AUTHOR_PASSWD}" --role=author --allow-root
