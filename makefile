...
COMPOSE_FILE := ./srcs/docker-compose.yml
MKDIR_CMD := mkdir -p $(VOLUME_DB) $(VOLUME_WEB)
RM_PS_CMD := docker rm -f $$(docker ps -aq 2>/dev/null) || true
RM_VOL_CMD := docker volume rm -f $$(docker volume ls -q 2>/dev/null) || true
RM_IMAGE_CMD := docker rmi -f $$(docker images -q 2>/dev/null) || true
